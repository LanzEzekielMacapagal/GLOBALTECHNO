...
server.put("/courses/:courseId/assignments/:assignmentId", upload.fields([{ name: "attachments", maxCount: 10 }]), async (req, res) => {
  try {
    ...
    const existingAttachments = Array.isArray(existingAssignment.attachments)
      ? existingAssignment.attachments.map((item) => ({ ...item }))
      : [];

    const removedAttachmentNames = parseRemovedAttachmentValues(payload.removedAttachments);

    const remainingAttachments = filterAttachmentsByRemoval(existingAttachments, removedAttachmentNames);

    const attachments = uploadedFiles.length
      ? [...remainingAttachments, ...uploadedFiles]
      : (Array.isArray(payload.attachments) ? payload.attachments : remainingAttachments);
